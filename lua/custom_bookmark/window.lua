...
function M.fill_border_data(buf, width, height, title)
  ...
  local middle_line = M.border_chars.MID_VERTICAL .. string.rep(" ", width) .. M.border_chars.MID_VERTICAL
  for _ = 1, height do
    table.insert(border_lines, middle_line)
  end
  table.insert(
    border_lines,
    M.border_chars.BOTTOM_LEFT .. string.rep(M.border_chars.MID_HORIZONTAL, width) .. M.border_chars.BOTTOM_RIGHT
  )
...
function M.create_win(opts)
  ...
  -- buf
  local win_buf_pair = create_win(opts.row, opts.col, opts.width, opts.height, opts.relative, opts.focusable, 256)

  return win_buf_pair
end

function M.create_border(opts)
  local border_win_buf_pair =
    create_win(opts.row - 1, opts.col - 1, opts.width + 2, opts.height + 2, opts.relative, false, 255)
  ...
  M.fill_border_data(border_buf, opts.width, opts.height, opts.title)

  api.nvim_win_set_option(border_win_buf_pair.win, "winhighlight", "Normal:" .. opts.border_highlight)
